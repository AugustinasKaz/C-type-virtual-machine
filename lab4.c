...
int running = true;
int ieof_flag = 0; // flag for EOF check
int duom_counter = -1;

void decode(command *arg_pc, char *duom, long duom_size);
command *fetch1(command *arg_pc); //used for commands calculation
command *fetch2(command *arg_pc); //used for JMP calculation
int change_counter(int type);     // used for reading txt file char
void rez_file(int symbol);        // output to file

int main()
{
    char prog_mem[256];
    command *arg_pc;
    char *duom; // txt file arr
    long duom_size;
    ...
    while (running)
    {
        ...
        { //check if current command is JFE and EOF
            arg_pc = fetch2(arg_pc);
            decode(arg_pc, duom, duom_size);
        }
        else
...
{
    arg_pc = (command *)((char *)arg_pc + arg_pc->cop1); //add constant to change from current command to next 
    return arg_pc;
}
...
void decode(command *arg_pc, char *duom, long duom_size)
{
    unsigned char regs[16];                  //arr to save register values
    int tmp_reg1 = arg_pc->cop1 & 0x0f;      //save commands first register number(0,1,2)
    int tmp_reg2 = arg_pc->cop1 & 0x0f >> 4; //save commands second register number(0,1,2)
